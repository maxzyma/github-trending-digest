...
在标题/副标题之后，**必须**有一段顶层洞察段，命名固定为 `## 本日主线`。条数自适应（1-N 条；榜单趋势分散就 1 条，多主题并发就多条），每条 1-2 段，含数据点 + 一句洞察判断。

```markdown
## 本日主线

**主线一：<主题名> — <一句结论>**。<段落正文：事实/数据/对照>。

**主线二：...**。

（如果当日榜单没有明显主线，写 1 条"今日榜单以 N 个 NEW 项目铺面为主，无统一主题"也算合规——重点是别强凑结构）
```

命名严格固定 `## 本日主线`，禁止用 `## 今日主线 / 本日主线 / 今日三主线 / 今日趋势观察 / 今日趋势洞察 / 今日核心信号 / 今日关键叙事 / 今日概览 / 今日纵览` 等同义异名变体——下游派生流程（钉钉文档摘要、dashboard 聚合）按段名解析，命名漂移会破坏链路。
...
文档写完后，在本目录 `index.md` 对应章节**下方第一行**插入新链接：
